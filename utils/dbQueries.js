...
function getEmployeesByDept() {
    return db
        .promise()
        .query(
            `SELECT d.dept_name, CONCAT(e.last_name, ', ', e.first_name) AS Employee, e.id, r.title, r.salary, CONCAT(m.last_name, ', ', m.first_name) AS Manager FROM employees e JOIN roles r ON e.role_id = r.id JOIN departments d ON r.department_id = d.id LEFT JOIN employees m ON e.manager_id = m.id ORDER BY r.id;`
        );
}
...
function getEmployeesByRole() {
    return db
        .promise()
        .query(
            `SELECT r.title, CONCAT(e.last_name, ', ', e.first_name) AS Employee, e.id, r.salary, CONCAT(m.last_name, ', ', m.first_name) AS Manager, d.dept_name FROM employees e JOIN roles r ON e.role_id = r.id JOIN departments d ON r.department_id = d.id LEFT JOIN employees m ON e.manager_id = m.id ORDER BY r.id;`
        );
}
